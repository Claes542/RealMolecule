...
atomic spectra fix, ${\approx}24$~meV, the metallic onset, where the pinning no longer holds and
...
the $\tfrac12$ that atomic calibration fixes to $1$, which
compensates for a free interface that resists neither compression nor modulation. The corrugation
falls elevenfold. Why an extended object is entitled to a coefficient an atom's spectrum does not fix
is argued in the text.

\textbf{What the corrugation gives, and what it does not.} In the activated regime it \emph{is} the
activation energy, so $\sigma\propto\nu\exp(-E_a/kT)$ is supplied except for the attempt frequency
$\nu$ --- and $240$ against $24$~meV is four orders of magnitude in conductivity at room temperature.
The missing $\nu$ is a lattice frequency, computable in principle here since the nuclei move under
classical equations of motion, though it is not computed in this article. What is genuinely outside
is the metallic case: $\sigma=ne^2\tau/m$ needs a momentum relaxation time from electron--phonon
scattering, and a minimisation over real densities has no current, no dynamics and no
irreversibility. The carrier throughout is a \emph{free boundary}, in the relation a dislocation
bears to slip.
\end{abstract}

\setstretch{1.15}

\section{The question, and the two formulations}\label{sec:intro}
...
carries the coefficient an atom's spectrum fixes rather than one an extended arrangement chose.
...
spectra, hydrogen first among them. It is fixed \emph{there}, on a bound electron in a single
...
the one an atom's spectrum fixes. Raising it is therefore not the introduction of a parameter but a
...
an atom's spectrum fixes. Supply that resistance and the pinning goes. Nothing here requires
