...
total energy of the system computed here those are $2\times10^{-3}$, $5\times10^{-5}$ and
$5\times10^{-6}$, so whether a material conducts is decided by a difference five or six orders of
magnitude below the number a calculation returns. The same span falls on the \emph{model}: a
description adequate for binding, where a per cent error is invisible, can be hopeless for
conduction, where a per cent of the total is a hundred times the quantity in question. Insulation is
then not a separate phenomenon but the absence of that difference.

\textbf{Two models, two verdicts.} A \textbf{chain of atoms} --- the valence electron occupying its own
...
insulator. A \textbf{cable of ions surrounded by electrons} --- ion cores along an axis, the carriers standing off
...
binding, hydrogen bonds and solvation, nuclear binding by dual Coulomb confinement
\cite{realnucleus} --- has been on \emph{bound} matter, where an attractor is always present and the
charge is where the attractor is. That is the regime the picture was built for, and it is where the
record stands. Conduction is the first question that asks about charge which is \emph{not} where an
attractor holds it, and the framework has no separate machinery for it: the same energy, the same
domains, in a different arrangement of the same two kinds of charge.

Two arrangements are put to it here, and they are ordinary matter rather than idealisations. The
first is a \textbf{chain of atoms}: one kernel per site with its valence electron occupying the same
region, which is what a hydrogen chain is and what an alkali is to a first approximation. The second
is a \textbf{cable of ions surrounded by electrons}: the kernels screened by their own core
electrons, and the valence charge standing off outside them --- which is how a wire is built. The
question is what each gives, and whether they give the same answer.

\paragraph{The difficulty is one of scale, and it falls on the model as much as on the arithmetic.}
Atomic energies are tens of electron-volts. Activation energies in the semiconductors and polaronic oxides this
...
without the error swamping it. Two things follow, and both shape the article. The
\emph{computation} must produce a difference of that size out of totals five or six orders larger,
which is why the controls here occupy as much space as the results and why more than one measurement
has been withdrawn. And the \emph{model} must be right at that resolution: a description adequate for
binding energies, where an error of a per cent is invisible, can be hopeless for conduction, where a
per cent of the total is a hundred times the quantity in question. Insulation, on this
...
\paragraph{The whole of the experiment, said in advance.} Two models are measured, and the reader is
owed that before the argument that surrounds it. The first is a \textbf{chain of atoms}: model atoms
in a row, each valence electron occupying its own kernel's region. Put it in a uniform field with the
partition held fixed and read the induced dipole, which gives $\alpha$ and how it scales with length;
then, with no field, displace the whole partition rigidly and record the energy, which gives the
periodic curve $E(u)$ whose amplitude is the corrugation and whose steepest slope is the field at
which the pattern would run. The second is a \textbf{cable of ions surrounded by electrons}: ion cores along an axis
with the carriers standing off them, where the same corrugation is obtained by displacing only the
middle carriers, so that the ends of a finite chain cancel out of the difference. Everything else is
control --- boundary conditions, spacings, fillings, a mesh ladder, and the requirement that a shift
by one full spacing return the same energy.
...
\section{Two models: a chain of atoms, and a cable of ions surrounded by electrons}\label{sec:three}
...
\midrule
chain of atoms: carrier on its own kernel & ${\sim}1$~eV & insulator; outside the hopping class\\
cable of ions: carriers standing off the cores & ${\approx}240$~meV & semiconductor range: activated hopping\\
