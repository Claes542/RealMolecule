...
\subsection{The muon as a compressed electron, and the meaning of mass}

Because RealQM carries \emph{no mass in its electrostatic content} --- structure follows from charge and a
kinetic coefficient $\kappa=\hbar^2/2m$ alone --- ``mass'' in RealNucleus has a purely \emph{inertial}
meaning: it is the inverse localisation (Compton size) of a charge cloud. A tightly localised cloud carries a
shorter matter-wave and more inertia; a spread cloud less. With \emph{no gravitation} in the model, this is
all that mass is: one may speak entirely of \emph{size}, $m\sim 1/\lambda$ with $\lambda$ the intrinsic
(Compton) size, and reserve the \emph{gravitational} meaning of mass (the gravitational charge) for a separate
sector. The proton/electron mass ratio is then a \emph{size} ratio: the proton is ${\sim}1836\times$ more
localised than the electron ($\lambda_p\approx0.21$ fm vs $\lambda_e\approx386$ fm), hence ${\sim}1836\times$
heavier.

This makes the two species different in \emph{kind}. The \textbf{proton is rigid} --- one fixed size, one
fixed mass. The \textbf{electron is fluid}. Two of its freedoms must be kept apart: in a Coulomb well its
\emph{bound extent} adapts to the environment (expanded in an atom, compact in a nuclear cage) at
\emph{fixed} rest mass; but the electron can also take different \emph{intrinsic} sizes, and these are the
\textbf{heavier leptons}. A muon is then not a separate elementary particle but the \emph{same electron charge
in a compressed state} --- intrinsic size ${\sim}207\times$ smaller ($\lambda_\mu\approx1.87$ fm), hence
${\sim}207\times$ heavier; the $\tau$ is a still more compressed state. The three ``generations'' become three
size-states of one electron.

The reading fixes the \emph{stability ordering} for free. The ordinary electron is the \textbf{ground state}
--- the most relaxed, least localised, lowest-mass cloud --- and is therefore \emph{stable}: there is nothing
lighter to relax into. The muon and $\tau$ are \emph{compressed, excited, metastable} states above it, so they
relax back down to the electron, shedding the excess mass-energy as radiation --- exactly
$\mu\to\mathrm{e}+\nu+\bar\nu$, lifetime $2.2\,\mu$s. As with the free neutron above (\S\ref{sec:neutron}),
instability is a higher-energy configuration decaying to a lower one, not the work of a dedicated force.

The same identification makes \textbf{muon-catalysed fusion} intelligible here: a compact (heavy) electron
forms a correspondingly compact bond (binding length ${\propto}1/m$), pulling two deuterons roughly two
hundred times closer --- into fusion range. Our muon simulations use exactly this scaling.

We flag the open points, in the honest spirit of the limitations below. The model does not derive \emph{why}
the compressed states sit at the particular ratios $m_\mu/m_e=206.8$ and $m_\tau/m_e=3477$ (what quantises the
compression), nor does it address lepton \emph{flavour} (distinct neutrino species, the strong suppression of
$\mu\to\mathrm{e}\gamma$) or the magnetic moments. As elsewhere, the claim is a reinterpretation within a
consistent size/charge ontology --- the muon as a compressed electron, mass as inverse size --- not a
derivation of the lepton spectrum.
