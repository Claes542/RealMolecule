...
barrier disappears is a property of $E(d)$ alone: $8.5\times10^{9}$ and $3.3\times10^{9}$~V\,m$^{-1}$
for the two arrangements, an order above the dielectric strength of silica, so both would break down
before they would slide.
...
conductivities. For
a sinusoid of amplitude $V_0$ the steepest slope is $\pi V_0/a$, so
\[
F_{\rm th} = \frac{\pi V_0}{ea}
= 8.5\times10^{9}~\text{V\,m}^{-1}\ \text{(chain)},\qquad
3.3\times10^{9}~\text{V\,m}^{-1}\ \text{(cable)} .
\]
Above that field charge advances without thermal help; below it, it does not advance at all without.
Both figures are an order above the dielectric strength of silica, which is the honest reading of
the pinning: these arrangements would break down before they would slide. The sinusoid enters here
...
A \emph{dissipation channel}. Ohm's law needs energy to be lost. A conservative minimisation gives
no current below threshold and unbounded acceleration above it, never a drift proportional to field.
The ingredients are not absent in principle --- the nuclei are classical, so the electron--lattice
channel other treatments import is present in the equations and $\tau$ would be an output rather
than an input --- but nothing of the kind is computed here.

Of the three, only the temperature is an import in the strong sense --- a statistics the framework
