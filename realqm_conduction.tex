...
interfaces. One functional carries the physics --- Coulomb potential energy without self-repulsion,
plus kinetic energy --- with nothing fitted, at a cost scaling as $N$ times the mesh.

It has been applied to solids, to molecular binding and solvation, and to nuclear binding --- always
to \emph{bound} matter, where an attractor holds the charge. The task here is conduction: the cost
of advancing the electron pattern of a solid by one lattice site. No such computation without fitted
parameters appears to have been carried out: the textbook $N$-electron state lives on a
...
one system, so the large errors cancel. Four cases are computed. A \textbf{chain of sixteen hydrogen
atoms} gives $394$~meV per electron, its minimum off registry near a quarter spacing --- so
advancing one site is a two-step passage. A \textbf{cable of sixteen lithium ions}, carriers
standing off closed cores, gives $246$~meV; stiffening them takes it to $20$~meV, below
...
intrinsic germanium, with attempt frequencies landing at lattice values unasked --- though these are
lower bounds and overestimates respectively. The barriers are what is computed.
\end{abstract}
