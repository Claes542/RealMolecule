...
lattice's residual is $5.3\times10^{-4}$, against $1.8\times10^{-3}$ for the chain and
$4\times10^{-3}$ for the cable, the smallest in this article, a partition into compact cells being
evidently closer to what the functional would choose. The cost is set by the grid rather than by the
number of atoms, and atoms added at fixed mesh are close to free. At the mesh used for the lattice,
$h=0.281\,a_0$, a $200^3$ grid spans $56\,a_0$, which at this spacing holds of order a thousand
atoms --- so a system of that size is a question of memory on a single GPU and not of a different
method. What it is not is quick: the eight-thousand-step relaxations reported above ran for hours
apiece on one machine, and a larger grid is proportionally worse. The limit on this calculation is
patience and the constraints it has to impose, not dimension.
