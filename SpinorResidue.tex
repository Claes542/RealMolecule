...
\paragraph{A conjecture on the connection.} If exclusion is geometric and the magnetic moment is a
separate SU(2) structure, then the spin--statistics \emph{connection} itself may be an artefact of
bundling the two under one name --- a feature of the standard formulation rather than a law of nature. We
state this as a conjecture, not a result: RealQM decouples what standard QM welds, and whether the weld is
necessary or merely conventional is left open.
...
consistency.} The spinor is a per-electron magnetic label, and it must coexist with the geometric
realisation of Pauli exclusion by non-overlap \emph{without double-counting the spin} --- concretely, a
filled shell of two electrons must pair to zero net moment (diamagnetism); the single-electron result does
not show this, and it is the real consistency test. \emph{(ii)~Why the spinor.} Using
