...
What is established is the \textbf{capability} gap: RealQM can \emph{run} folding dynamics at protein scale;
DFT cannot. The remaining frontier is \textbf{accuracy} --- how faithfully RealQM's energy surface drives the
chain to the correct native state unaided (dispersion, a folding driver, is a known RealQM weak point, and
current runs may use biases). That concerns the \emph{fidelity of the fold}, not whether the dynamics can be
run, and it does not revive DFT, which is disqualified by cost regardless.
