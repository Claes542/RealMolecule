...
\subsection{Ions: ionization potential of Li from total-energy difference}\label{sec:ions}

A direct test of the framework on ions is to compute the first ionization potential of Li as the difference between two independent ground-state runs: neutral Li (3 electrons, 1s$^2$ inner halfspaces plus an outer 2s sphere) and Li$^+$ (2 electrons, 1s$^2$ inner halfspaces only). Both run on identical 200$^3$ grids spanning a 12~a.u.\ box ($h = 0.06$~a.u.) with kernel softening $r_{\rm soft}^2 = 1.5\,h^2$:
...
Both totals are within $\sim 1\%$ of observation. The ionization energy comes out at $0.24$~Ha vs the experimental $0.198$~Ha (overestimate $\sim 20\%$); the residual is dominated by grid coarseness for the tight Li$^+$ 1s$^2$ shell ($\langle r \rangle \sim 0.33$~a.u., only $\sim 5$ points across at $h = 0.06$). A finer 300$^3$/12~a.u.\ run ($h = 0.04$) tightens the Li$^+$ binding further; the IP converges from above as the grid refines. The point is methodological: in RealQM, an ionization energy is just the difference of two independent variational ground states, with no separate ``ionization'' machinery.
