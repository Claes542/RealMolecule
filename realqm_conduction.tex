...
the start removes the need for them, at a cost that grows linearly in $N$ rather than exponentially:
nine times the electrons on one and the same grid are measured here to cost $8.92$ times the time.
That is the new capability reported here: the quantity is computed rather than modelled.
...
stiffened --- and the barrier to advancing the electron pattern by one site differs by a factor of
thirty between them. None is a one-dimensional model. The wave functions are functions
...
the nuclei, and in the fourth they form a cubic crystal. A \textbf{chain of hydrogen atoms},
...
standing off closed cores, gives ${\approx}246$~meV. A \textbf{cubic lattice} of $72$ hydrogen atoms
gives ${\approx}678$~meV, an estimate rather than a value: the windowing that removes the chain ends
exactly in the first two is not applied to it, and on the chain that same omission overstated the
barrier by some forty per cent. It is reported mainly for what it settles --- it makes the nuclei a
crystal rather than a line, so the carrier density becomes a property of the structure instead of a
convention, and it is the best-conditioned calculation here. The first two are the computed values, at the coefficient
...
cost of minimising \eqref{eq:E} grows \emph{linearly} in $N$ at a fixed mesh, and that is measured
here rather than asserted: nine times the electrons on one and the same $148^3$ grid --- the lattice
of \S8 against a single chain drawn from it --- costs $8.92$ times the time per relaxation step.
The reason is the representation. Each $\psi_i$ is a function on three dimensions, so $N$ of them
cost $N$ grids, where the single wave function of textbook quantum mechanics lives on a
$3N$-dimensional configuration space and costs one grid raised to the power $N$. Linear against
...
indeed with the smallest residual in this article --- and why the hundred-atom lithium lattice noted
below could be run under its own dynamics at all. The measurement above is made on seventy-two
electrons and should be read as such, but it is consistent with that larger case and with the sizes
the applications listed below have reached. That is the framework. The
...
standing off outside them, which is how a wire is built. The third, in \S8, is a \textbf{cubic
...
whether it beats a percentage. Nothing in \eqref{eq:E} is adjusted to reproduce a transport
...
content should not be judged by the accuracy of ones that do.
...
its boundaries \emph{are} free: solids have been computed that way, ionic crystals through melting
and shear and a lithium lattice of a hundred atoms recovering from a perturbation under its own
dynamics \cite{code}. What is fixed here is a geometry of a kind the free minimisation produces,
...
The barrier for the lattice is an \emph{estimate} only, and the reason is the ends. The windowing
of \S3, which removes them exactly in \S4 and \S5 by differencing two window sizes, is not applied
here: the lattice slides globally, so its difference between $d=0$ and $d=\half$ carries the two
junctions along with the bulk. The size of that is known from the chain, where the same omission
gave $642$~meV against the $394$~meV the windowed measurement returns --- a difference of some
forty per cent, in the direction of overstating the barrier. On that basis the lattice's
${\approx}678$~meV should be read as an upper estimate of a bulk quantity, not as one, and it is
not comparable with the windowed values quoted elsewhere in this article. The slid end is also not
converged, as the table shows, and the sequence is not monotone, so it cannot be extrapolated
either.
...
where \S6 must state it as a convention. Its barrier is an estimate only --- ${\approx}678$~meV,
unwindowed and so carrying its junctions, which on the chain inflated $394$ to $642$ --- and
supplying a real one means windowing and scanning the lattice as the chain and cable are.
...
evidently closer to what the functional would choose. The scaling is the linear one of \S1, measured on this system: nine
times the electrons at one mesh cost $8.92$ times the time per step, so atoms are paid for once each
and not compounded. At the mesh used here, $h=0.281\,a_0$, a $200^3$ grid spans $56\,a_0$ and holds
of order a thousand atoms at this spacing, which is within the memory of a single consumer GPU; by
the same linear law such a run would take days rather than the hour and a half the lattice took. A
...
is $5.7$~Ha from its converged value, and on the lattice the ladder of \S8 still moves the barrier
between $4000$ and $8000$ steps while $E(0)$ has stopped changing in the sixth figure. Shortening the
