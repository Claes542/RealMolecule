...
the problem rather than inputs to it. The cost of that minimisation grows \emph{linearly} in $N$:
nine times the electrons on one and the same grid are measured here to cost $8.92$ times the time,
where the single wave function of textbook quantum mechanics, living on a $3N$-dimensional
configuration space, would cost one grid raised to the power $N$.
...
the start removes the need for them. That is the new capability reported here: the quantity is computed rather than modelled.
