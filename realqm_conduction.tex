...
coefficient of the kinetic energy above the $\half$ an atom sets --- takes the barrier through $kT$
at about $1.75$ times that value, and to nothing by twice it, with the preferred registry, carriers
on their own ion sites, unchanged throughout.
...
bounds for a reason that does not depend on the mechanism: the barrier is a minimum over the paths
joining two registries and the rigid slide is one such path. A kink is the candidate for the cheaper
one, but none is computed here. A resistive conductivity is outside altogether,
...
What the cheaper path \emph{is} remains a question this article does not settle. The expected
answer, and the one the rest of this section assumes, is a kink --- one carrier advancing
...
\paragraph{A first three-dimensional lattice.} That step has been taken for the simplest case.
Hydrogen has no core and no standoff, so unlike the cable there is no lower bound on how close
neighbouring chains may sit --- the cable needs $c_{\rm sep}\gtrsim1.6\,a$, since its cores occupy
$2c_r$ and its carriers need room between them --- and hydrogen atoms can therefore be placed on a
simple cubic lattice at the same spacing $a$ in all three directions. Seventy-two of them,
$3\times3\times8$ at $a=4.5\,a_0$, one electron per kernel and nothing imposed beyond the lattice
itself:
\[
E(0)=-37.742193,\quad E(\half)=-36.023663~\text{Ha},\qquad
\varepsilon\approx650~\text{meV per electron},
\]
against $642$~meV for the chain of eight --- a difference of about one per cent.

The total energy is not similarly insensitive: $-0.524$~Ha per atom against the chain's $-0.453$,
and below even the exact free atom's $-0.5$, because a domain in the lattice is a compact cell of
volume $a^3$ rather than a slab running out to the box face. Six neighbours instead of two therefore
change the binding substantially and the barrier hardly at all. That says the pinning is
\emph{local} --- set by an electron's own kernel and the two it moves between, not by the transverse
environment --- and it answers by measurement a question \S1 can otherwise only answer by
description: arranging the nuclei in three dimensions returns the line's barrier to one per cent.
It also makes $n=1/a^3$ exact for this arrangement, where \S6 has to state it as a convention.

The lattice is the better-conditioned calculation, which was not expected of nine times the domains:
its residual at $d=0$ is $5.3\times10^{-4}$, against $1.8\times10^{-3}$ for the chain and
$4\times10^{-3}$ for the cable, the smallest in this article. A partition into compact cells is
evidently closer to what the functional would choose than one into elongated slabs.

Two checks stand and one does not, and the figure of $650$~meV should be read accordingly. The
construction is verified three ways. At $1\times1$ it reproduces the chain of \S4 to all nine
digits. The nuclear sum over the $72$ protons agrees with an independent evaluation of the same
lattice to eleven figures, so the arrangement is the one intended. And the translation control
holds exactly: at a matched step count $E(d{=}1)$ and $E(d{=}0)$ agree to every digit reported,
which they must, a displacement of one full spacing being a relabelling of the same calculation.
What is \emph{not} settled is relaxation. The residual at $d=\half$ is twenty-one times that at
$d=0$, where the chain's two ends agree within a factor of two, and an under-relaxed state sits too
high. So $650$~meV is at present an upper bound on this lattice's barrier rather than a measurement
of it, and the comparison with the chain's $642$ should be read as the two agreeing to about one
per cent from above.
...
Stiffening the carriers of that cable takes the barrier through $kT$ at about $1.75$ times the
coefficient an atom sets, without changing the preferred registry. So the barrier is set
