...
\begin{itemize}
\item[(A)] \emph{Retain $\Psi\in H^1$.} Density continuity $\rho_1=\rho_2$ is kept as a constraint at all
times, so the moving interface must satisfy it \emph{together with} the velocity continuity
\eqref{eq:vmatch}. That is a condition beyond the material law, and it over-determines the interface unless
the fields arrange to meet both; density continuity does not persist under \eqref{eq:material} on its own.
\item[(B)] \emph{Take velocity continuity as the dynamic interface law.} The interface is then a genuine
contact discontinuity: per-electron charge conservation and \eqref{eq:vmatch} govern it, a density jump is
allowed, and $\Psi$ acquires a kink at a moving interface --- no longer strictly $H^1$. This is the natural
charge-conserving choice, and it reduces to the static Bernoulli condition at rest, where $\mathbf v_i=0$
makes \eqref{eq:vmatch} trivial and density continuity becomes the substantive statement.
\end{itemize}
Position (B) is the more economical: it follows from the symmetries of the action alone, with no added
function-space constraint, at the price of relaxing the $H^1$ regularity that static RealQM assumes;
position (A) keeps that regularity but must carry an extra interface condition whose compatibility with
charge conservation is not guaranteed. We record the fork and leave its resolution --- a choice about the
time-dependent theory, to be decided by which alternative the physics of real-time multi-domain dynamics
in fact realises --- as open.
