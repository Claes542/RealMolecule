...
difference between a conductor and an insulator?} Where the textbook formulation represents $N$
electrons by one wavefunction on $3N$ dimensions, RealQM represents them by $N$ non-negative unit
charge densities in ordinary three-dimensional space, one per electron, meeting at free boundaries
and minimising the Coulomb energy --- with nothing fitted and polynomial rather than exponential
cost. The established answer to the question is band theory: fill the bands of a periodic potential
and count, which is correct, cheap and applicable at the size of a crystal, but supplies the
potential rather than deriving it, as density functional theory supplies a functional. The question
here is whether a framework starting further back, from the charges and their Coulomb interaction
alone, reaches the same distinction by its own route.
...
materials show. And when the carriers are given the stiffness against density modulation that a
degenerate electron gas has, it falls to ${\approx}24$~meV, thermal energy, and the pinning is gone.
...
\paragraph{And the framework is missing exactly that.} A degenerate electron gas resists density
modulation because compressing it costs Fermi energy. Here a flat density is admissible on a free
