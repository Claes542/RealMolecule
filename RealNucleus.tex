...
saturation are electromagnetic; no strong force is required.} Nor is a \emph{weak} force
required for instability: the model ``neutron'' ($\mathrm{1p}+\mathrm{1e}$) is computed
\emph{unbound}, so free-neutron $\beta$-decay into a proton and an electron emerges from the same
Coulomb energetics, with no weak-interaction term. The model is solved by signed
...
\mathrm{p}+\mathrm{e}^-+\bar\nu$ is mediated by the weak force acting on an otherwise elementary
neutron. In RealNucleus there is no elementary neutron to decay: the model ``neutron'' is just a
