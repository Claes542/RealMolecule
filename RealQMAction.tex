...
\section{Relation to the Bernoulli condition of static RealQM}
\label{sec:bernoulli}

Static RealQM equips the free boundary with a \emph{Bernoulli condition}: continuity of the fields across
the interface, $\psi_1=\psi_2$ on $\Gamma$ (forced by $\Psi=\sum_i\psi_i\in H^1$), together with the
homogeneous Neumann condition $\partial_n\psi_i=0$, the latter arising as the transversality condition when
the Coulomb energy is minimised over the domains \cite{RealQM}. The two formulations meet, and it is worth
being exact about how.

In the \emph{static} limit they coincide. For a stationary state the currents $\mathbf j_i$ vanish, so
\eqref{eq:material} gives $V_n=0$: the boundary is at rest, and \eqref{eq:action} reduces to the very
energy minimisation whose transversality condition is the Neumann law. The stationary-action principle
therefore \emph{recovers} the Bernoulli condition of static RealQM in exactly the regime where static
RealQM is used --- atoms, molecules, and nuclei.

Out of equilibrium the two must be distinguished, and there are two genuine subtleties.

First, the \emph{density-continuity} half of the Bernoulli condition, $\rho_1=\rho_2$ on $\Gamma$, does
\emph{not} appear among the equations derived above. It is not a consequence of the domain variation but a
constraint on the admissible function space, imposed in static RealQM by requiring $\Psi=\sum_i\psi_i\in
H^1$. What the variation yields at the interface is instead \emph{velocity} continuity,
\eqref{eq:vmatch}, from per-electron charge conservation. These are different conditions: a material
interface permits a density jump (a contact discontinuity), which would violate $\Psi\in H^1$. Requiring
both --- the $H^1$ density continuity of static RealQM and per-electron charge conservation --- with the
boundary in motion is not automatic, and reconciling them is precisely the open point.

Second, and relatedly, the static Neumann condition applied literally in real time would force $\mathbf
j_i\!\cdot\!\mathbf n=\mathrm{Im}(\psi_i^*\partial_n\psi_i)=0$ and hence, by \eqref{eq:material}, $V_n=0$: a
\emph{moving} material boundary is incompatible with strict Neumann. Neumann is a property of the
\emph{static} transversality; the time-dependent transversality condition --- from including the temporal
terms of \eqref{eq:action} in the domain variation --- differs from it and permits motion. Per-electron
charge conservation fixes the \emph{kinematics} of the moving boundary (it is material, \eqref{eq:material});
the precise dynamic field condition that replaces static Neumann and density continuity for a genuinely
moving interface is a point we identify but do not settle here. In the regime where RealQM has so far been
applied --- ground states and Born--Oppenheimer relaxation --- neither distinction arises, since there the
boundary is at rest, the currents vanish, and the Bernoulli condition (density continuity plus Neumann)
holds.

These two subtleties are in fact one choice, and it is worth stating it sharply. The material interface
\eqref{eq:material}--\eqref{eq:vmatch} is, in the language of fluid mechanics, a \emph{contact
discontinuity}: the normal velocity is continuous across it while the density may jump. Time-dependent
RealQM must then adopt one of two positions.
It is worth being clear that density continuity is nothing exotic: since $\rho_i=|\psi_i|^2$, continuity of
the density is simply continuity of $\psi^2$, which follows at once from $\psi$ being continuous, i.e. from
$\Psi\in H^1$. Continuity of $\psi$ fixes its \emph{value} at the interface, and hence $\rho_1=\rho_2$; the
velocity, by contrast, is the phase \emph{gradient} $\mathbf v=\nabla S$, and continuity of $\psi$ says
nothing about the gradient. Density continuity (of the value) and velocity continuity (of the gradient) are
therefore independent conditions. The two positions are then:
\begin{itemize}
\item[(A)] \emph{Retain $\Psi\in H^1$.} This is the natural, economical condition --- the wavefunction is
continuous, so $\rho_1=\rho_2$ holds automatically. The question is whether one may \emph{also} impose
per-electron charge conservation, i.e. the velocity continuity \eqref{eq:vmatch}: that is a condition on the
gradient, over and above the continuity of the value, and requiring both may \emph{over-determine} the
interface, since density continuity does not by itself persist under the material law \eqref{eq:material}.
\item[(B)] \emph{Give up $\psi$-continuity to conserve charge.} The interface becomes a genuine contact
discontinuity: per-electron charge conservation and \eqref{eq:vmatch} govern it, a jump in $\psi$ (hence in
$\rho$) is permitted, and $\Psi$ is no longer strictly $H^1$. This is the charge-conserving choice; it
reduces to the static Bernoulli condition at rest, where $\mathbf v_i=0$ makes velocity continuity trivial
and density continuity is the substantive statement.
\end{itemize}
So the fork is between keeping the wavefunction continuous (A), at the risk that per-electron charge
conservation cannot then also be imposed, and enforcing charge conservation (B), at the price of a jump in
the wavefunction. We record it and leave its resolution --- a genuine choice about the time-dependent theory
--- open.
