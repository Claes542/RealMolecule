...
\label{eq:tdrealqm}
\end{equation}
with the dot denoting differentiation in time and $H_j$ the single-particle Hamiltonian seen by electron $j$ (kinetic plus kernel plus the Hartree potential built from the other electrons). This is structurally a wave equation in the same sense that the standard time-dependent Schr\"odinger equation is a wave equation, an analog of $u_{tt} + u_{xx} = 0$ but first order in time and complex valued.
