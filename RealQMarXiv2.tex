...
\normalsize\textit{Version 2: extended condensed-phase calibration with CO$_2$ dry ice and solid N$_2$}}
...
In this respect RealQM is a direct realisation of \textbf{Dirac's view of chemistry as applied quantum physics}: the famous 1929 remark that ``the underlying physical laws necessary for the mathematical theory of [\ldots] the whole of chemistry are thus completely known, [and] the difficulty is only that the exact application of these laws leads to equations much too complicated to be soluble.'' RealQM takes Dirac's program seriously by retaining only the underlying physics --- Coulomb interactions between electron densities and nuclei --- and discarding the chemistry-specific machinery (basis sets, exchange--correlation functionals, fitted force fields) that the standard reformulations introduce in order to make the equations tractable. Chemistry then re-emerges, bottom-up, from the variational principle alone.
...
E(Z) \sim -Z^{7/3},
\end{equation}
a result derived from the Thomas--Fermi model rather than the full Schr\"odinger equation. Empirical observations across the periodic table give a slower scaling $E(Z) \sim -Z^\alpha$ with $\alpha \approx 1.8$ --- much closer to $Z^2$ than to $Z^{7/3}$.
...
\end{equation}
Used by Kato in his analysis of the Schr\"odinger equation, the inequality bounds the singular Coulomb potential energy at a single nucleus by a function of the kinetic energy. The total wavefunction in RealQM is a sum
\begin{equation}
\Psi(\mathbf{x}) = \sum_{n=1}^{N} \Psi_n(\mathbf{x})
\end{equation}
of electronic wavefunctions $\Psi_n$ with \textbf{non-overlapping supports} meeting at the Bernoulli free boundary (continuity plus homogeneous Neumann). Each $\Psi_n \in H^1(\mathbb{R}^3)$, and so does $\Psi$. Applying the Hardy inequality~\eqref{eq:hardy} to each $\Psi_n$ separately and summing, one obtains the stability bound
\begin{equation}
E \;\ge\; -C\, N\, Z^2,
\end{equation}
with $C$ an absolute constant --- this is the ``stability of the second kind'' of the Dyson--Lenard--Lieb--Thirring tradition.

\textbf{The non-overlap constraint is the physical origin of stability in RealQM.} Each electron's potential-energy contribution is bounded by its own kinetic energy on its own region of support; no global book-keeping is required because there is no overlap between distinct electrons. Stability is a direct consequence of the formulation rather than a deep theorem to be proved separately.

\paragraph{Atomic energy scaling: $Z^2$ vs $Z^{7/3}$.}
A more refined analysis yields the leading-order scaling of the total electronic energy of a single atom with nuclear charge $Z$. The unit-density formulation produces an approximate shell structure with the following scaling laws (derived in detail elsewhere): shell radius $r_m \sim m^3/Z$, shell thickness $d_m \sim m^2/Z$, $2m^2$ electrons in the $m$-th filled shell, and charge density $\rho_m(r) \sim Z/r^2$ in spherical symmetry --- giving constant shell charge per unit radial distance. Integrating the kinetic + potential contributions over the resulting density yields
\begin{equation}
E(Z) \;\sim\; -\log(Z)\, Z^2,
\end{equation}
which is essentially $-Z^2$ up to the logarithmic correction. This agrees well with observed atom energies for $Z < 30$ outside the first shell and for larger $Z$ outside the second shell, and is in substantially better agreement with observation than the Lieb--Thirring $-Z^{7/3}$ bound.

\paragraph{Significance.}
Stability of matter, viewed in StdQM as one of the deepest theorems of mathematical physics, becomes in RealQM a one-paragraph consequence of the Hardy inequality applied to non-overlapping electronic territories. The contrast is informative: \textbf{the proof difficulty in StdQM is not a feature of the underlying physics but a symptom of the formulation.} Working in $3N$-dimensional configuration space with overlapping orbitals forces the singularities of the Coulomb potential to be controlled globally, requiring the elaborate apparatus of Dyson--Lenard--Lieb--Thirring. Working in real 3D space with non-overlapping unit densities localizes the control, and stability falls out of the variational structure. The simpler argument also delivers the better $Z^2$ scaling against observation.
