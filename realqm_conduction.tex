...
\begin{abstract}
\noindent RealQM represents an $N$-electron system by $N$ real-valued wave functions $\psi_i(x)$, with $x$ a coordinate in
...
It has been tested against ionisation energies across the periodic table, molecular binding,
hydrogen bonds and solvation, and nuclear binding by dual Coulomb confinement --- all of it on
\emph{bound} matter, where an attractor is present and the charge is where the attractor is.

The task posed to it here is conduction, which is the first question asked of it about charge that
is \emph{not} where an attractor holds it: what does it cost to advance the electron pattern of a
solid by one lattice site, which is the activation energy for charge transport. The answer is obtained from Coulomb interaction and kinetic energy
...
standing off closed cores, gives ${\approx}246$~meV. Those two are the computed values, at the coefficient
...
What is evaluated, here and throughout, is $E(d)$ at \emph{two} values of $d$, together with $d=1$
as a control. Symmetry places stationary points at $d=0$ and $d=\half$ --- the curve is periodic and
inversion-symmetric about a site, so $E(\half+x)=E(\half-x)$ --- and \eqref{eq:V0} is then read as
$E(\half)-E(0)$. The barrier so obtained is a difference of two computed numbers and assumes nothing
about the shape of the curve between them. Two later quantities do assume it: the curvature $K$ that
sets the attempt frequency, and the steepest slope that sets the threshold field, both of which are
taken from a sinusoid of the computed amplitude. A scan at intermediate $d$ would replace the
assumption with a measurement, and would at the same time confirm that $d=\half$ is the maximum
rather than a second minimum with the true peak between. That scan has not been made.
...
answer the corrugation below returns.

A quantitative comparison against that equation of state has not been made here and should be, but
two things stand in the way of it and both are worth stating.

The first is the nuclear cutoff of \S9. It cancels between two configurations of one system, which
is what the corrugation is and why the barriers survive it; it does \emph{not} cancel between a
chain and a free atom, which is what a binding energy is. Measured, over a cutoff interval of
$0.26\,a_0$, the chain moves $123$~meV per atom and the free atom $46$: the chain electron is
pressed against its nucleus by its neighbours and carries more density inside the cutoff. Nor can
the cutoff simply be made small. It is bounded below by twice the mesh spacing, so shrinking it
means refining the grid, and at the smallest value reachable the chain comes out more bound per atom
than the exact H$_2$ molecule --- which is not a prediction but a calculation that has stopped being
trustworthy, the cutoff by then spanning about two cells. A comparison of binding energies therefore
awaits the nuclear cusp being treated analytically rather than truncated, which is solver work not
attempted here.

The second is what such a comparison could establish even if the arithmetic were clean. Those calculations are the essentially
exact solution of the non-relativistic clamped-nucleus problem, and for hydrogen that is as close to
the physical answer as computation gets, relativistic corrections being of order $10^{-5}$~Ha. What
they are not is a measurement, and what they represent is the prediction of the theory RealQM is
offered as an alternative to. A disagreement would therefore be ambiguous in a way it would not be
for an approximate solution of the same equations: it could mean this framework is wrong, or that
the two descriptions differ for this arrangement, and no experiment on an eight-atom hydrogen chain
exists to decide between them. It would be a consistency check against the best available solution
of the standard theory, on a system where that theory is unusually trustworthy --- worth having, and
worth not overstating. The tests against measurement are the ones already in the framework's record,
and they are on bound matter. This is the simplest arrangement in which
...
\begin{tabular}{@{}rrrr@{}}
\toprule
steps & residual at $d=0$ & residual at $d=\half$ & barrier\\
\midrule
...
\noindent The unslid end is converged --- it moves by $0.007$~meV per electron over the last
...
The defensible statement is therefore narrow. At the best-relaxed pair available the lattice gives
${\approx}678$~meV per electron against the chain's $642$, some five per cent above it, with the
slid end still moving and the approach to it irregular. That the two agree to five per cent while
the binding per atom differs by sixteen is suggestive of a barrier set locally rather than by the
transverse environment, and it is no more than suggestive. What would settle it is further steps on
the slid end, which at $610$~ms per step on $72$ domains is machine time rather than method.
