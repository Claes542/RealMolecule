...
amplifies small scales ($\rho_k\propto k^2\varphi_k$); the seed itself is a \emph{small fluctuation}.
From this one seed the universe develops in \textbf{two tiers}: \textbf{large-scale gravitation} ---
positive mass aggregating into the \textbf{cosmic web}, negative mass repelling as \textbf{dark energy}
--- and \textbf{small-scale electromagnetics} --- the co-seeded $\pm$charge, with $+$ tied to the
\textbf{large mass (protons)} and $-$ to the \textbf{small (electrons)}, forming nuclei and atoms. As
\textbf{gravitational contraction} makes the positive mass hot and dense, \textbf{deuterons and nuclei}
form (${\sim}$MeV, RealNucleus) --- the step required for everything beyond hydrogen --- and, on cooling,
\textbf{atoms} (${\sim}$eV, RealQM); the electromagnetic \textbf{binding energy is released as radiation}
--- the origin of \textbf{light} --- the mass defect feeding back to gravity via $E=mc^2$. The
electron is taken to have \textbf{two temperature-selected size-phases} --- compact (inside nuclei,
${\sim}$MeV) and expanded (atomic, ${\sim}$eV) --- so a single cooling curve orders nuclei then atoms, with
...
$T_\ast$, with \textbf{no strong force, no elementary neutron, no matter--antimatter asymmetry, and no
rebounce or singularity} (a finite-fluctuation origin). We state the picture, its one quantitative handle
($T_\ast$), and its load-bearing open problems --- chiefly \emph{what sets the nuclear scale}, and whether
the $\grad^2\varphi$ \emph{fluctuation spectrum} matches large-scale structure.
...
\medskip
\noindent\textbf{Keywords:} cosmology; two-tier cosmology; induced gravity; proton--electron model of
...
\textbf{two-tier} cosmology --- large-scale gravitation and small-scale electromagnetics from one seed
fluctuation --- with dark energy and a unified treatment of nuclear and atomic structure built in, and
with no large-scale rebounce or singularity. We develop it as a chain of postulates and consequences, and we keep the open
problems explicit.
...
\textbf{How the Coulomb sector couples to gravitation --- only through energy.} Gravity never sees electric
charge directly; it sees only \emph{energy}, and $E=mc^2$ turns the Coulomb sector's energy (binding, kinetic,
radiation) into \emph{mass}, the gravitational charge --- realised in matter (each cloud carries both a charge
and a mass) and manifested in the \emph{mass defect} (binding lowers the energy, hence the mass; the difference
radiates). $E=mc^2$ is itself neither from RealQM nor from Newton but a bridge imported from the wave/radiation
side ($P=mc$, largely a convention) --- the one place $c$ enters; see the foundations note \emph{Where
$E=mc^2$ comes from}.
...
\end{enumerate}
The Laplacian's $k^2$ amplification first makes many \emph{small} $\pm$mass regions, already spanning a
range of magnitudes (deep wells $=$ larger positive mass, shallow $=$ smaller). \textbf{Charge is co-seeded
at this same small scale, not added later}: one correlated fluctuation sources both potentials, so the
co-aligned electric potential --- nonzero only on the positive-mass regions --- assigns $+$ charge to the
\textbf{larger} positive mass (protons) and, by \emph{local neutrality}, $-$ charge to the \textbf{smaller}
positive-mass remainder (electrons), leaving negative mass \textbf{chargeless}. Net charge is zero already
at the small scale, and strong electromagnetism makes this \emph{fast}, quantizing it to $\pm1$
(Section~\ref{sec:quantization}). Only \emph{then} does the weak, \emph{slow} gravitational aggregation act:
same-sign mass attracts and opposite repels, so the small-scale $\pm$mass accumulates into a
\textbf{large-scale web} --- positive-mass filaments and nodes (carrying their already-neutral
proton--electron charge) and negative-mass regions, separated by voids from the repulsion. So the genuine
order is \textbf{fast local seeding, then slow aggregation}; mass and charge themselves are simultaneous
(correlated), not sequential (Sections~\ref{sec:bounce},~\ref{sec:quantization}).

\section{Contraction, nucleosynthesis, and dark energy}\label{sec:bounce}
The sequence needs no large-scale collapse-and-rebounce; ordinary gravitational contraction supplies the
one hot dense epoch, and the released energy leaves as light:
\begin{enumerate}
\item \textbf{Gravitational contraction $\to$ hot dense.} The positive-mass seed --- a \emph{small
fluctuation} --- \textbf{contracts gravitationally} into a hot dense state. The hot dense comes from the
contraction, not from the seed.
\item \textbf{Deuterons and nuclei, and light.} The contracting matter is already a proton--electron mix
(charged at the seed, Section~\ref{sec:origin}); the mass difference is gravitational, distinct from the
electrostatic kinetic coefficient $\kappa$, which treats both as charge clouds for atomic/nuclear
structure. The hot dense state builds \textbf{deuterons and nuclei} (${\sim}$MeV, RealNucleus) ---
required for everything beyond hydrogen --- and the electromagnetic \textbf{binding energy is released as
radiation (light)}, the mass defect feeding back to gravity via $E=mc^2$. No rebounce is invoked: the
energy leaves as light, not as a mechanical bounce.
\item \textbf{Cooling $\to$ atoms.} As it cools, nuclei capture expanded electrons to
form \textbf{atoms} (${\sim}$eV, RealQM), releasing radiation. Hydrogen needs no nucleosynthesis;
heavier atoms need the nuclei above.
\item \textbf{Slow large-scale aggregation, and dark energy.} Much later, weak gravity aggregates the
positive mass into the \textbf{cosmic web}; the negative-mass regions are \emph{chargeless} --- no
electromagnetics, no light, hence literally \textbf{dark} --- acting only by gravity and repelling as
\textbf{dark energy}. This negative mass is \emph{not antimatter} (antimatter is a charge mirror, with
\emph{positive} mass); it is a chargeless, repulsive \emph{gravitational} sector, and the model invokes no
antimatter. Electric structure and light live only in the visible (positive-mass) region.
\end{enumerate}
Two features stand out: the picture has \emph{no singularity} (a finite-fluctuation origin, in the spirit
of bounce cosmologies~\cite{bounce}) and is \emph{two-tier} --- small-scale
electromagnetics (charge, nuclei, atoms, light; early) and large-scale gravitation (contraction to the hot
dense state, then the cosmic web; late). It keeps ordinary gravitational contraction but needs \textbf{no
rebounce and no bounce energy budget}: the released energy radiates as light rather than reversing a
collapse. \textbf{Cosmic expansion has two drivers}: an \textbf{initial expansion from the seed} itself,
and the \textbf{ongoing acceleration from dark negative-mass repulsion}.

\paragraph{One seed, two scales --- a micro--macro bridge.} Both potentials are seeded by the \emph{same
small-scale} fluctuation (the Laplacian amplifies small scales). The electric seed builds the small-scale
world directly (charge $\to$ atoms $\to$ nuclei); the gravitational seed --- although gravity only
\emph{acts} on large scales --- is \emph{also} small-scale at origin, and grows into the large-scale cosmic
web by slow aggregation. So the largest structures and the smallest share \textbf{one small-scale origin}:
a connection between small and large scales, in the spirit of a \textbf{Real Theory of Everything}.

\paragraph{A 2D test: cosmic-web morphology, and voids from negative-mass repulsion.}
A mass-conserving 2D simulation of the mass seed and its aggregation --- generate mass $\rho=\grad^2\varphi$ from a random potential
fluctuation, then evolve compressible self-gravitating flow with same-sign attraction and opposite-sign
repulsion --- develops the \textbf{morphology of observed large-scale structure} (Figure~\ref{fig:web}):
two \textbf{intermixed filamentary webs} (positive- and negative-mass), mass \textbf{concentrated at nodes}
where filaments meet, separated by \textbf{large voids}. Filaments and nodes arise from same-sign
gravitational aggregation; crucially, the \textbf{voids are actively driven by the negative-mass repulsion}
(the dark-energy sector), rather than being merely under-dense regions evacuated by gravity as in
$\Lambda$CDM. This is a \emph{qualitative} morphological match (a 2D toy); a quantitative test (void-size
distribution, filament statistics, the two-point correlation function against surveys and $\Lambda$CDM)
remains to be done.
\begin{figure}[h]\centering
\includegraphics[width=0.5\textwidth]{cosmic_web_pm2.png}
\caption{2D self-gravitating $\pm$mass from $\rho=\grad^2\varphi$: intermixed positive- (warm) and negative-
(cool) mass webs with concentrated nodes and large voids --- the cosmic-web morphology, with voids driven by
negative-mass repulsion. Mass-conserving; periodic box.}\label{fig:web}
\end{figure}

\section{Why charge is quantised and mass is not}\label{sec:quantization}
\emph{In one line:} the large positive mass gets the $+$ charge; the $-$ charge is the
\textbf{neutrality-mandated remainder}, carried by the small positive mass --- \textbf{that is the basic
asymmetry} (charge is co-seeded with mass, so charge sign tracks mass size; which sign we call $+$ is the
global labeling convention).

The construction places charge \emph{onto} the mass, not independently of it. Mass sets the template ---
the gravitational fluctuation makes \textbf{deep wells} (large, concentrated mass) separated by
\textbf{shallow regions} (small, diffuse mass) --- and the electric fluctuation is \emph{co-aligned} with
it (one correlated primordial fluctuation sourcing both potentials, not two independent ones). The $+$
charge then lands on the deep wells (the \textbf{heavy protons}), and charge neutrality forces the $-$
charge into the shallow complement (the \textbf{light electrons}). The electron is thus the
\emph{neutrality-mandated leftover}, and it is necessarily small and diffuse --- both because the
big-mass sites were already claimed by the $+$ charge, and because, being spread out, it has large size
and hence (mass $\sim$ inverse size) small mass. The correlation \textbf{big${}\leftrightarrow{}+$,
small${}\leftrightarrow{}-$} is a consequence, not a separate assumption; which sign we call $+$ is the
global labeling convention. (This is correlated \emph{seeding}, not a time sequence: since
electromagnetism is ${\sim}10^{39}$ times stronger than gravity, charge cannot wait for mass to finish
segregating --- the two must be laid down already correlated. Neutrality then keeps the newly charged
scaffold bound: protons plus electron clouds carry no net long-range force, so charging does not blow the
structure apart.)

This same strong/weak asymmetry explains a fact usually taken as given --- \textbf{charge is quantised in
exact units, mass is not}:
\begin{center}
\begin{tabular}{lll}
\toprule
 & charge & mass \\
\midrule
force & strong (EM) & weak (gravity, ${\sim}10^{39}\times$ weaker) \\
dynamics & fast & slow \\
outcome & discrete $\pm1$ units & continuous gradation \\
symmetry & exactly equal and opposite & grossly unequal ($1836{:}1$), top-heavy \\
\bottomrule
\end{tabular}
\end{center}
\textbf{Fast $\Rightarrow$ quantised.} Strong electromagnetism drives opposite charges together fast (the
Coulomb collapse of Section~\ref{sec:temp}); stabilised by the electron's kinetic energy, they lock onto
the one \emph{stable bound unit}, the proton--electron pair, which carries \textbf{integer charge} ($+1$
core, $-1$ cloud). All charge therefore relaxes onto multiples of a single elementary unit, and $+e$ and
$-e$ come out \emph{exactly} equal and opposite because they are the \emph{same} unit --- quantisation as
fast relaxation onto the stable charged state.
\textbf{Slow $\Rightarrow$ gradation.} Gravity is weak, slow, and --- crucially --- has \emph{no stable
elementary unit to snap onto}: same-sign mass simply keeps accreting, nothing halting it at a preferred
size. Mass therefore never quantises; it accumulates by gradation into a \textbf{continuous spectrum
weighted toward the big wells} (most of the mass in nuclei, and upward to stars). The two 2D simulations
embody the contrast directly: the charge run (fast collapse onto discrete bound $\pm1$ pairs) versus the
mass run (slow segregation into a continuous graded web).

Two honest limits remain. The fast dynamics explains why charge appears in \emph{integer multiples} of a
unit, but not why the unit $e$ exists (a stable $\pm e$ soliton is still an input); and at the elementary
level protons and electrons do have definite masses, so the ``continuum'' is really the \emph{aggregate}
mass spectrum --- the $1836$ ratio itself stays the open number of Section~\ref{sec:open}.
...
atom formation & expanded $e$ captured $\to$ atoms; radiation & ${\sim}3000$ K ($kT{\sim}$eV) \\
\bottomrule
\end{tabular}
...
the hot dense state --- is handled within RealQM \emph{deterministically},
...
($k^2$-amplified) density spectrum, whereas large-scale structure fits a near
...
\item \textbf{Precision data and the rest.} BBN abundances are
...
cooling-quench using the engine's temperature and Brownian-dynamics machinery to
...
more than its precise magnitude. The standard view is that $\alpha$ must lie in a \textbf{two-sided window}
--- neither \textbf{too large} (else the electromagnetic self-energy makes the proton outweigh the neutron
and hydrogen decays, and heavy atoms destabilise at $Z\alpha\to1$) nor \textbf{too small} (feeble chemistry,
unviable stars) --- a few percent wide for the strict carbon/oxygen (Hoyle) resonance, a factor of a few for
the looser atom/star criteria. Two remarks. First, the \emph{existence} of the nuclear/atomic hierarchy is
far more robust to $\alpha$ than the detailed universe (which elements form, how stars burn). Second,
\textbf{one side of that window dissolves here}: since neutron $=$ p $+$ e, the neutron is
\emph{structurally} heavier than the proton --- by construction, independent of $\alpha$ --- so proton and
hydrogen stability is \textbf{automatic, not tuned}, and the mainstream ``$\alpha$ must not be too large or
the proton decays'' bound simply does not arise.
