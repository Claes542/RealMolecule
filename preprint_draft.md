...
**Bonding mechanism.** The bond is established by **accumulation of electron density between the two kernels**. In the unit-density formulation, the two non-overlapping electron territories meet at the free boundary between the kernels, where **both densities are non-zero** — neither is required to vanish at the interface, in contrast to the exponential decay to zero at infinity for an isolated atom. This non-vanishing meeting (the Bernoulli condition of Section 2.4: continuity plus homogeneous Neumann) admits a configuration in which the **kinetic energy is locally increased** in the inter-nuclear region, an increase that would be forbidden if the densities had to decay to zero between the nuclei. The accumulated density lowers the total energy through enhanced Coulomb attraction to both nuclei, and the variational principle accepts the kinetic-energy cost in exchange for the larger Coulomb gain. Covalent bonding in RealQM is thus a direct consequence of the free-boundary structure: the absence of a vanishing-density requirement at the inter-electron interface is what makes inter-nuclear density accumulation possible, and that accumulation is the bond.
