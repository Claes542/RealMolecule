...
A parallel deterministic program exists for thermal radiation. In a series of notes~\cite{Johnson2010BB} one of us has proposed a classical wave model
$u_{tt} - u_{xx} - \gamma u_{ttt} = f$
in which an Abraham--Lorentz radiative damping term $-\gamma u_{ttt}$ replaces photon statistics, with a high-frequency cutoff supplied by finite-precision discretisation rather than by Planck quantisation. Spectral analysis of the damped oscillator yields the Rayleigh--Jeans law $R \propto \nu^2 T$ directly from the PDE; the cutoff at $\nu \approx kT/h$ removes the ultraviolet catastrophe without invoking quanta. The structural parallel is exact: in both cases (matter and radiation) the move is to replace a probabilistic statistical postulate with a deterministic continuum equation plus a regularisation length/precision scale, and to recover the macroscopic law as a property of the regularised PDE rather than as the average of an underlying random process. RealQM and this blackbody picture are siblings of the same anti-statistical program.
