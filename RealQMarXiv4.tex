...
The analytical results just discussed --- the Hardy stability bound~\eqref{eq:bulk-bound}, the boundedness of minimising sequences in $H^1(\mathbb{R}^3)$ from the kinetic-energy bound~\eqref{eq:Kbound}, and the free-boundary regularity transferable from~\cite{AltCaffarelli81, AltCaffarelliFriedman84, CaffarelliLin08, HHOT09} --- establish the analytical scaffolding for existence and regularity of the RealQM ground state in a weak sense. What remains for a complete analytical theory is the adaptation of these results to the specific RealQM setting: Coulomb singularities at each nucleus (handled locally by Hardy), and the coupling between phases through the Hartree potentials $V_j$ in~\eqref{eq:Hi}, which is the structural feature that distinguishes RealQM from the autonomous multiphase systems of Caffarelli--Lin. These adaptations are routine in scope but technically substantial, and have not been written down in full anywhere.

The numerical solutions reported in this paper carry part of the load that the missing analytical theory would otherwise carry. The 3-line code of Section~\ref{sec:3line}, applied to atoms, molecules, condensed phases, and protein-scale systems, reaches converged solutions in which the wave functions $\psi_i$ stabilise to spatial profiles consistent with the variational equations~\eqref{eq:Hi}--\eqref{eq:neumann}, the Bernoulli free boundary stabilises with continuity and homogeneous Neumann holding to mesh resolution, and the total energy plateaus. \textbf{Each such converged simulation may be regarded as a constructive proof, to a specified numerical precision, that a solution of the variational problem exists, and that it is regular enough to be represented on the simulation grid.} This is in the spirit of computer-assisted proofs (the 4-colour theorem, the Kepler conjecture, parts of mathematical fluid dynamics): a problem too complex for closed-form analytical resolution is settled, in the cases of interest, by an explicit construction whose correctness is verifiable. The constructive evidence does not replace the eventual analytical work that adapts~\cite{CaffarelliLin08, HHOT09} to the coupled-Coulomb setting, but it carries the same epistemic load in the cases where individual systems are at issue --- and anyone who doubts that a particular system has a well-defined RealQM ground state can run the corresponding scan file from the public Gallery and inspect the converged result directly.
