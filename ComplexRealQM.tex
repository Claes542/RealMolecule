...
\mathbf J_i = q_i\,\frac{\hbar}{m_i}\,\mathrm{Im}\!\left(\psi_i^*\nabla\psi_i\right),
\qquad \rho=\sum_i\rho_i,\ \ \mathbf J=\sum_i\mathbf J_i .
\label{eq:rhoJ}
\end{equation}
The evolution is \emph{unitary} on each domain,
\begin{equation}
i\hbar\,\partial_t\psi_i \;=\; \mathcal H_i\,\psi_i,
\qquad
\mathcal H_i = -\frac{\hbar^2}{2m_i}\nabla^2 + V_i ,
\label{eq:tdse}
\end{equation}
...
$\mathbf J_i\!\cdot\!\mathbf n = q_i(\hbar/m_i)\,\mathrm{Im}(\psi_i^*\,\partial_n\psi_i)$, and the Bernoulli
...
partition. (For small-amplitude motion the domains may be held fixed at their ground-state shape; for
large-amplitude dynamics the boundary co-evolves, a harder problem noted in \S\ref{sec:open}.)
...
$\psi_i = a\,\phi_1 e^{-iE_1t/\hbar}+b\,\phi_2 e^{-iE_2t/\hbar}$, has a density that beats at
$\omega=(E_2-E_1)/\hbar$ and an oscillating dipole $\mathbf d(t)=\int\mathbf x\,\rho\,d\mathbf x$, which
...
Zitterbewegung structure --- carrying angular momentum $\hbar/2$ and moment $\sim\mu_B$. Equivalently, one
...
boundary, now with a complex phase evolved by $i\hbar\dot\psi_i=\mathcal H_i\psi_i$. The free boundary
