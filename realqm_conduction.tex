...
the electron on its own kernel, gives ${\approx}394$~meV per electron and a polarisability
...
$2.6\times10^{8}$~V\,m$^{-1}$ for the three arrangements. The first two are above the dielectric
...
$0.1$ and $10$~S\,m$^{-1}$, which bracket intrinsic germanium at $2$~S\,m$^{-1}$, with attempt
frequencies at $h\nu=76$ and $23$~meV, lattice energies that were not asked for. The barriers are
...
The barrier is also far less sensitive to the one arbitrary length in the construction than the total
energy is: moving the nuclear cutoff from $0.6$ to $0.8\,a_0$ shifts the total by $6.2$~eV and the
barrier by $40$~meV, a suppression by a factor of $158$ --- the error cancellation of \S3 shown
rather than argued, and the honest size of the uncertainty, some $10\%$ of the barrier from the
cutoff alone.
...
No barrier is quoted for the lattice. Two things prevent it. The slid end is not converged, as the
table shows, and the sequence is not monotone so it cannot be extrapolated. More seriously, the
lattice slides \emph{globally}: the windowing of \S3 is not applied to it, so its two junctions do
not cancel and its difference carries the ends, which is the fault that cost the chain its first
number. A value obtained that way is not comparable with the chain's $394$~meV, which is windowed,
and setting the two against each other would compare an end-contaminated quantity with a clean one.
What the lattice establishes is therefore structural --- that the construction is verified three
ways, that the binding per atom rises from $-0.453$ to $-0.524$~Ha on going from a line to a
crystal, and that $n=1/a^3$ is exact for it --- and not a barrier. Supplying one means windowing the
lattice as the chain and cable are windowed, and scanning it.
...
barrier of $394$~meV per electron for the electron pattern to advance one site, and a polarisability
