...
\begin{abstract}
We compare the ontology of an atomic system of $N$ electrons under two models. \emph{Standard} quantum
...
\label{eq:std}
\end{equation}
Two metaphysical difficulties follow directly, one from each feature of \eqref{eq:std}.
...
\label{eq:real}
\end{equation}
with $\rho_i=q_i\psi_i^2$ the charge density of electron $i$ on its own region $\Omega_i\subset\mathbb R^3$,
$q_i=-1$, and $\rho=\sum_i\rho_i$ the total charge density. The ground state minimises the Coulomb energy
\begin{equation}
...
the whole matter ontology and \eqref{eq:E} its law. Two caveats: \eqref{eq:real} is not equivalent to
\eqref{eq:std} --- non-overlap is not antisymmetry --- so this is a rival theory, not an interpretation;
and its warrant is the quantitative chemistry and nuclear structure it reproduces \cite{RealQM}, its reach
bounded to those scales.
...
One feature sets \eqref{eq:real}--\eqref{eq:E} apart from the metaphysical proposals of \S\ref{sec:two}: it
is \emph{constructive and computable}. Wave-function realism and the primitive-ontology programme are
readings of a formalism one solves the standard way, on $\mathbb R^{3N}$; the ontology is interpretive,
supplied after the calculation. In \eqref{eq:real}--\eqref{eq:E} the beables \emph{are} the objects of
computation: one minimises the energy \eqref{eq:E} directly over the charge densities $\psi_i$ in
$\mathbb R^3$, and the primitive ontology carries the calculation rather than glossing it --- complexity
scaling with mesh points in $\mathbb R^3$, not with the exponential dimension of $\mathbb R^{3N}$. The
...
proton--electron reading --- nuclear binding, all as browser-runnable computation \cite{RealQM}. The
...
the same fact, and Pauli exclusion and the identity of indiscernibles coincide. Whether such a theory is
empirically adequate is for physics; conditional on adequacy in its atomic and nuclear domain, its
metaphysics is, on these two long-contested points, the cleaner one.
