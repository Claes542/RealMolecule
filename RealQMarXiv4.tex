...
The Hardy inequality~\eqref{eq:hardy} also bounds the kinetic energy from above at any bound state, without invoking the virial theorem. The argument is short. From~\eqref{eq:hardy} applied to a unit-norm $\psi_i$ on $\Omega_i$,
\begin{equation}\label{eq:Vbound}
|V_{eK}| \;\le\; \sum_a Z_a \int \frac{\Psi^2}{|\mathbf{x} - \mathbf{R}_a|}\, d\mathbf{x}
\;\le\; \Bigl(\textstyle\sum_a Z_a\Bigr)\, \sqrt{2K},
\end{equation}
where we used $\|\Psi\|_2^2 = N$ for an $N$-electron system and absorbed the $\sqrt{N}$ into the constant. The inter-electron repulsion is non-negative ($V_{ee} \ge 0$). Combining with the energy decomposition $E = K + V_{eK} + V_{ee}$,
\begin{equation}
K \;=\; E - V_{eK} - V_{ee} \;\le\; E + \Bigl(\textstyle\sum_a Z_a\Bigr)\sqrt{2K}.
\end{equation}
Setting $u = \sqrt{2K}$ and rearranging gives the quadratic inequality $\tfrac{1}{2}u^2 - (\sum_a Z_a)\, u - E \le 0$, which for any finite $E$ implies
\begin{equation}\label{eq:Kbound}
K \;\le\; \tfrac{1}{2}\Bigl(\textstyle\sum_a Z_a + \sqrt{(\sum_a Z_a)^2 + 2E}\Bigr)^{\!2}.
\end{equation}
