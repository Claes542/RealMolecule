...
standing off closed cores, gives ${\approx}246$~meV. The same cable with the carriers stiffened ---
the coefficient of the kinetic energy doubled --- gives ${\approx}1$~meV, a fall by a factor of some
$240$: the pinning is removed, while the preferred registry, carriers on their own ion sites, is
unchanged.
...
$\nu$ of $2.3\times10^{13}$ and $5.5\times10^{12}$~Hz, that is $h\nu$ of $97$ and $23$~meV ---
lattice frequencies, not asked for --- and room-temperature
conductivities of $1\times10^{-5}$ and $10$~S\,m$^{-1}$, which bracket ordinary semiconducting
material --- intrinsic silicon at $4\times10^{-4}$ and intrinsic germanium at $2$ lie between them. For the stiffened cable the expression does not apply, a barrier at $0.04\,kT$
being no barrier at all. What the framework supplies is the exponent; the nuclear mass and the
...
\[
\varepsilon_{\rm bulk} \approx +1~\text{meV per carrier},
\]
and the collapse is the result: a fall by a factor of some $240$ from the unstiffened cable, to
$0.04\,kT$. The preferred registry is unchanged --- the carriers still sit on their own ion sites
--- so what the stiffening removes is the pinning itself and not merely its location.

That is what a stiff carrier lattice should do. A lattice that resists being modulated resists being
pulled into the ion wells, and past some stiffness the cost of following them exceeds the cost of
keeping its own spacing uniform.

The number is small enough to need saying what it does and does not establish. One meV is a
difference of $1.5\times10^{-4}$~Ha between two total energies of $158$~Ha, and while the windowed
difference cancels everything common to the two configurations, this protocol has not been shown to
resolve a residual that small. What is established is the collapse, two and a half orders of
magnitude and far outside any drift the step ladder shows; the precise value is not, nor is the
exact location of the minimum, the response resting on two values of the coefficient rather than a
curve and on the two endpoints $d=0$ and $d=\half$.

An earlier version of this section reported $-31$~meV here, with an inverted registry in which the
carriers preferred to sit between the ion sites rather than on them. That was an artifact of the
grid. The ion kernels were then rounded to whole cells, and since $a/h=8.0129$ the spacings came out
$8,8,8,\ldots,9$ cells --- a lattice irregular by up to $6\%$ of a spacing. The spurious corrugation
that introduced was larger than the real one and of the opposite sign. The episode is worth
recording rather than quietly removing, because it bounds what may be asked of this kind of
quantity: a barrier two orders below the arrangement's own energy scale is delicate against
geometric defects that leave every symmetry of the problem intact, and is therefore not settled by
any symmetry control.
...
five values of the distortion now resolves them. Neither quantity is measured in this article.
...
Stiffening the carriers of that cable collapses the barrier to ${\approx}1$~meV, a factor of some
$240$, without changing the preferred registry. So the barrier is set
...
unstiffened and $+1.0$~meV stiffened.
