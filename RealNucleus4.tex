...
the Coulomb model under a single calibration. The comparison is thus a parameter-free
\emph{prediction} placed against a model-free \emph{measurement}: a test that hides neither its
successes (nothing is fitted) nor its failures (the target is weighed, not modelled).
...
\textbf{empirical fact about the nucleus, on the same footing as a weight read from a scale}.
...
throughout. The model cannot hide behind theory: its output is checked against a benchmark that
owes nothing to any nuclear model. Its successes are therefore unhidden, \emph{because nothing was
fitted to obtain them}; and its failures are equally unhidden, \emph{because the target was
measured, not modelled}. The same measured benchmark that certifies the parameter-free agreement
