...
compact electron). A Saha-style balance with $T_\ast$ as the single fitted number is the next
calculation (Section~\ref{sec:comp}).
...
\begin{enumerate}
\item \textbf{The nuclear scale.} Equal-mass Coulomb is scale-free; a compact (${\sim}$fm) electron
needs an effective negative ``glue'' of ${\sim}5\times10^4\,m_e$ (a mass of $1836$ gives only
${\sim}10^3$, and is carried by the protons, not the glue; the size of a bound complex is set by its
lightest constituent). Candidate fix: take the proton's fixed ${\sim}$fm size as a fundamental input
length. This is what the strong force supplies in standard physics; it is the make-or-break gap.
...
field equation.
\item \textbf{Equal mass vs.\ $m_p/m_e=1836$.} The measured ratio (ten digits) contradicts equal mass
unless an effective mass emerges from structure.
...
heavy negative glue binds two protons. \textbf{Open:} the two-scale test (does one engine give MeV
\emph{and} eV in the same units? equal-mass Coulomb returns ${\sim}1$, confirming a scale ingredient
is needed); the $n/p$-vs-$T$ calculation yielding the helium fraction (Section~\ref{sec:helium}); and
a cooling-quench ``mini Big Bang'' using the engine's temperature and Brownian-dynamics machinery to
