...
The RealQM model can be extended to describe interaction matter--radiation following a principle presented at \url{https://computationalblackbody.wordpress.com} and developed in~\cite{Johnson2010BB}, which can be expressed as a classical wave equation with an Abraham--Lorentz radiative damping term:
\begin{equation}
u_{tt} - u_{xx} - \gamma u_{ttt} = f.
\end{equation}
Here $f$ is incoming radiation forcing, and $-\gamma u_{ttt}$ is the radiation reaction on accelerating charge density. The damping term replaces photon statistics, and a high-frequency cutoff is supplied by finite-precision discretisation rather than by Planck quantisation. Spectral analysis of the damped oscillator yields the Rayleigh--Jeans law $R \propto \nu^2 T$ directly from the PDE; the cutoff at $\nu \approx kT/h$ removes the ultraviolet catastrophe without invoking quanta. The structural parallel to RealQM is exact: in both cases (matter and radiation) the move is to replace a probabilistic statistical postulate with a deterministic continuum equation plus a regularisation length/precision scale, and to recover the macroscopic law as a property of the regularised PDE rather than as the average of an underlying random process. RealQM and the blackbody wave model are siblings of the same anti-statistical program.
...
notes (2010); see also \emph{Generic Model of Interaction Matter-Radiation},
