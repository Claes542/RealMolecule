...
\paragraph{The normalisation is legitimate, and that makes the objection worse.} It might be thought
that setting $\nu=1$ removes the phenomenon, since all the structure in \eqref{eq:grad} is structure
in $\nu$. It does not. Navier--Stokes rescales as
\[
v(x,t)=\alpha\,u(\beta x,\alpha\beta t),\qquad \nu'=\nu\beta/\alpha,
\]
so any viscosity can be set to one by rescaling the data, and small $\nu$ with moderate data is the
same problem as $\nu=1$ with large data. Fefferman asks about \emph{all} Schwartz-class initial
data, so arbitrarily high Reynolds number is inside the hypotheses.

The consequence is not that the formulation is saved but that it is worse off. Turbulent data are
covered, and a turbulent solution that never blows up is, in the technical sense, \emph{smooth}. So
the first horn of the dichotomy would come out true --- and say nothing whatever, because smoothness
does not distinguish laminar flow from turbulence. A theorem asserting global smoothness with
derivatives bounded by an iterated exponential is a theorem about a fluid in the way that a bound of
$10^{10^{10}}$ metres is a statement about the diameter of this room. The vacuity is in what the
dichotomy asks for, not in the normalisation.
