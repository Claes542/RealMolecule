...
\title{RealNucleus: Nuclear Binding as Dual Coulomb Confinement,\\ without a Strong Force}
...
\begin{abstract}
\textbf{RealQM} is a parameter-free, real-space computational model of quantum mechanics in
which charge appears as a sum $\Psi=\sum_i\psi_i$ of one-particle components, each carrying
unit charge on a non-overlapping domain $\Omega_i\subset\mathbb{R}^3$ separated by free
(Bernoulli) boundaries, with the ground state minimising the \emph{Coulomb} energy over the
components and the partition~\cite{Johnson-RealQM,Johnson-Book,Johnson-Atoms}. This article carries the
same model into the \textbf{atomic nucleus}. In the proton--electron picture --- a neutron is
a proton plus an electron --- a nucleus of mass number $A$ and charge $Z$ is $A$ protons and
$A-Z$ electrons. We treat both as the \emph{same} kind of object: equal-mass charge clouds on
non-overlapping domains, differing only in the \emph{sign} of the charge ($+1$ for protons,
$-1$ for electrons), interacting through \textbf{Coulomb forces only}. \textbf{No strong force
is introduced.}

The binding mechanism is \textbf{dual confinement}: the negative electron clouds hold the
positive protons together against their mutual repulsion, while the positive protons hold the
electron clouds together against theirs --- each species confining the other. We find that this
purely electromagnetic packing reproduces the scale of nuclear binding. Calibrating a single
energy unit on the deuteron ($\mathrm{2p}+\mathrm{1e}$, $2.22$~MeV), the alpha particle
($\mathrm{4p}+\mathrm{2e}$) emerges at \textbf{103\%} of its experimental binding with no
fitting, and the alpha/deuteron binding ratio at $13.1$ versus the experimental $12.7$. The
ladder $^2\mathrm{H}\to{}^4\mathrm{He}\to{}^8\mathrm{Be}\to{}^{16}\mathrm{O}$ binds as
\emph{single} connected Coulomb-confined units --- more bound than the same nucleons held apart
as separate alpha clusters --- with the near-linear (volume) growth of binding, and the marginal
stability of $^8$Be against breakup into two alphas, both appearing. The over-binding of the heaviest member at
its most compact configuration is removed not by any new force but by the nucleus
\emph{adding shells} as it grows: a spread $2{+}2{+}2{+}2\mid 4{+}4{+}4{+}4$ structure places
$^{16}\mathrm{O}$ at ${\sim}106$--$108\%$ of experiment. \textbf{Both the binding and the
saturation are electromagnetic; no strong force is required.} The model is solved by signed
...
This makes precise why \emph{neither} species can be spherically symmetric --- the central
structural fact of the model. The electron glue must break spherical symmetry to sit \emph{among}
the protons; a spherically symmetric electron core has no preferred protons to hold. Less
obviously, the protons cannot be spherically symmetric either: by the shell theorem a uniform
spherical shell of charge produces a \emph{constant} potential in its interior and exerts no force
on the charges it encloses, so a spherical proton shell would fail to confine the electron core at
all. It is precisely because the protons are a small number of \emph{discrete} clouds --- four
quarter-pieces for the alpha --- that their interior potential is \emph{not flat}
(Fig.~\ref{fig:confinement}, green curve), leaving a residual field that pulls the electrons
outward and cages them. Dual confinement thus requires \emph{both} distributions to break
spherical symmetry; the fully spherical configuration --- the one a one-dimensional radial
reduction is restricted to --- does not bind.

\begin{figure}[t]
\centering
\includegraphics[width=0.72\textwidth]{confinement22e4p.png}
\caption{Dual confinement in the alpha, the main He-4 configuration: the two electrons split
into \emph{halfspaces} (red/blue) and the four protons into \emph{quarter pieces}. Red is the
potential generated by the electrons --- a deep central well that pulls the protons inward.
Green is the potential generated by the protons; note that it is \emph{not flat} inside the
electron core. By the shell theorem a spherically symmetric proton shell would give a constant
interior potential and exert \emph{no} force on the electrons; but the four discrete protons are
not spherically symmetric, leaving a residual interior field that pulls the electrons outward
toward the protons, depleting the core, and so cages them. Each species is held only by the
other's Coulomb potential. The split of the two electrons into halfspaces is what enables the
binding: a spherically symmetric electron core does not bind.}
...
\section{Discussion: no strong force}
...
The picture used here is, in outline, an old one. Before Chadwick's discovery of the neutron in
1932~\cite{Chadwick1932}, the nucleus was generally taken to consist of protons \emph{and
electrons} --- a nucleus of mass $A$ and charge $Z$ being $A$ protons and $A-Z$ electrons,
exactly the bookkeeping we adopt. That proton--electron model was abandoned for three reasons:
...
elementary neutral particle removed the need for nuclear electrons, and Yukawa's meson
exchange~\cite{Yukawa1935} supplied the short-range strong force that has underpinned nuclear
binding ever since. That the nucleus cannot be held together by electromagnetism --- that an
