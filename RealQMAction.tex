...
Out of equilibrium the two must be distinguished, and there is a genuine subtlety. The static Neumann
condition, applied literally in real time, would force $\mathbf j_i\!\cdot\!\mathbf n=
\mathrm{Im}(\psi_i^*\partial_n\psi_i)=0$ and hence, by \eqref{eq:material}, $V_n=0$: a \emph{moving}
material boundary is incompatible with strict Neumann. The resolution is that Neumann is a property of the
static transversality, whereas the time-dependent transversality condition --- obtained by including the
temporal terms of \eqref{eq:action} in the domain variation --- differs from it and permits motion. The
per-electron charge conservation of \S\ref{sec:boundary} fixes the \emph{kinematics} of the moving boundary
(it is material, advecting with the flow); the precise dynamic field condition that replaces static Neumann
for a genuinely moving interface is a point we identify but do not fully settle here. In the regime where
RealQM has so far been applied --- ground states and Born--Oppenheimer relaxation --- the distinction does
not arise, since there the boundary is at rest and the Bernoulli condition holds.
