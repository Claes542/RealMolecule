...
\chapter{Prelude: Entropy and Turbulence}
...
internal energy as expressed by \emph{Euler's equations}. RNS is a least squares residual stabilised finite element method, which produces "best possible" computational solutions of Euler's equations in cases where pointwise exact solutions with zero residuals do not exist because of the unavoidable presence of turbulence and shocks.   
...
\section{RNS as Model of Thermodynanics}

In this book we extend the scope of \cite{ambsflow} to compressible flow described by the Euler equations solved by RNS into \emph{Computational Thermodynamics}.  We thus use RNS in an exploration of basic processes of thermodynamics in heat engines, heat pumps and refrigerators with turbulent dissipation setting limits of efficiency. 

\section{Icarus Education}

This book is part of \emph{Icarus Education} originating in the 
\emph{Body$\&$Soul} educational project \cite{bodysoul} and including:
\begin{itemize}
\item Computational Calculus.
\item Computational Turbulent Incompressible Flow \cite{ambsflow}.
%\item Computational Thermodynamics or Computational Turbulent Compressible Flow Vol 5.
\item Mathematical Theory of Flight.
\item Mathematical Theory of Sailing.
%\item Computational Solid Mechanics.
\item Mathematical Physics of Blackbody Radiation.
\item Many-Minds Relativity
\item Many-Minds Quantum Mechanics
\item Dr Faustus of Modern Physics.
\item The Clock and the Arrow: A Brief Theory of Time.
\end{itemize}


%The companion Icarus Ebook
%Vol 2 \emph{The Clock and the Arrow: A Brief Theory of Time} \cite{arrow},
%develops a variety of aspects of thermodynamics
%in a synthesis of philosophy, science and literature 
%directed to a wider audience.
%Vol 6 and Vol 5 can be read independently or together. 

%To really digest this book it is natural to first go through 
%Computational Turbulent Compressible Flow presenting fundamental aspects of 
%computational turbulence, together with relevant parts of Computational Calculus
%or the predecessor \cite{cde}. 
%To make it possible to directly plunge into this book,
%we make an overlap with the last Part VI of Vol 4.
\section{RNS Software: FEniCS: Unicorn}
Open source software implementing RNS is available as 
the \emph{FEniCS} application \emph{Unicorn} \cite{fenics}.

\section{Outline}

Part I Short Story recapitulates the questions and answers of classical thermodynamics 
and briefly presents the new answers offered by computational thermodynamics. 
Joule's experiment from 1845 with a gas expanding to double volume is used to illustrate a 
basic problem of thermodynamics and its resolution by RNS.

Part II World of Thermodynamics briefly covers some of the very many applications of thermodynamics from small
to large scales.

Part III Mathematics presents a basic mathematical analysis of solutions the Euler/Navier-Stokes equations, which we follow up in Part IV Computation with an analysis of RNS. The 2nd Law is proved and the limits of mean-value determinism are given 
by a posteriori error estimation based on solving a dual lineraized problem.

Part V Model Analysis is a study of some model problems by analytical mathematics.

Part VI Applications reaches the main goal of computing the efficiency of heat engines, heat pumps and refrigerators 
in prototype form. 


%We then continue with a survey of 
%classical thermodynamics and the role of thermodynamics in a variety
%of applications from small to large scale. We then derive 
%basic energy estiamtes for viscosity solutions of the Euler equations
%%as a perparation for the following presentation of computational 
%thermodynamics as the main theme of the book. We conclude with 
%a variety of applications showing the potential of computational
%thermodynamics.


%We show that the 2nd Law (\ref{secondlaw0}) is a consequence of the 1st Law 
%in the form of the Euler equations combined with RNS
%RNS introduces effects of viscosity as a consequence of the non-existence 
%of pointwise solutions to the Euler equations with zero residuals 
%reflecting instablities leading to the development shocks and turbulence 
%in which large scale kinetic energy is transferred to small scale
...
{T_1}
%\includegraphics[height=3.0cm]
%{imagethermofenics/Rho_left.eps}
%\includegraphics[height=3.0cm]
%{imagethermofenics/Rho_right.eps}
} 
\caption{Mean density and temperature in left and right chambers as functions of time.}
\label{densitytemp} 
\end{figure}
\begin{figure}[bhpt] 
\centerline{ 
...
\includegraphics[height=4.5cm]
{K_1}
\includegraphics[height=4.50cm]
{P_1}
%\includegraphics[height=3.0cm]
%{imagethermofenics/K_left.eps}
%\includegraphics[height=3.0cm]
%{imagethermofenics/K_right.eps}
} 
\caption{Mean kinetic energy and pressure in left and right chambers as functions of time.}
...
\caption{Mean turbulent dissipation in left and right chambers as functions of time.}
\label{turbdiss} 
\end{figure}
%\begin{figure}[bhpt] 
%\centerline{ 
...
\begin{figure}[bhpt] 
\centerline{ 
\includegraphics[height=7.0cm]
{Tem_T_3}
%\includegraphics[height=4.0cm]
%{joul_paraview/Tem_T_6.eps}
%\includegraphics[height=3.0cm]
%{joul_paraview/Tem_T_12.eps}
...
{vel_T_3}
%\includegraphics[height=4.0cm]
%{joul_paraview/vel_T_6.eps}
%\includegraphics[height=3.0cm]
%{joul_paraview/vel_T_12.eps}
%\includegraphics[height=2.0cm]
%{ambsthermopict/joule-pics/joule-tem-rc.eps} 
} 
...
the \emph{viscous heating} $\sigma\!:\!\nabla u\ge 0$ --- the irreversible
return of the jet's kinetic energy to heat on which the chapter's narrative
turns.

\section{Energy and Entropy Bookkeeping}
...
with the gas entropy
\[
S=\int \rho\,(\ln T-\gamma\ln\rho)\,dx ,
\]
whose increase $\Delta S=S(t)-S(0)>0$ is the quantitative 2nd Law. The entropy
production measures the irreversibility, concentrated in the channel jet and in
the recompression against the far wall.
...
A_0=\gamma\int \rho\,\ln(\rho/\bar\rho)\,dx \;=\; T_0\,\Delta S_\infty ,
\end{equation}
with $\bar\rho$ the mean density; the second equality is Gouy--Stodola, since the
spontaneous free expansion eventually destroys exactly this $A_0$ as
$T_0\,S_{\mathrm{gen}}$. The cooling delivered is the heat removed from the cells
...
destroying almost all of the available work $A_0$ in irreversibility while
delivering only a sliver of cold. This is exactly why a practical refrigerator
does not free-expand but runs the recompression cycle of the next chapter,
recovering work and driving $S_{\mathrm{gen}}$ toward the Carnot limit. The same
bookkeeping --- $A_0$, $Q_{\mathrm{cold}}$, $\Delta S$ --- will let us assign an
efficiency to any continuum-thermodynamic process, including the thermodynamics
of the living cell.
...
$R_e(\hat u)=\nu (\Delta u)^2$ expresses in pointwise form
...
\chapter{Thermodynamics of Reactive Flow}
...
\centerline{\psfig{figure=ambsthermopict/figCHARS.eps,height=2.5in,width=4in}}
