...
\begin{abstract}
\noindent Can the difference between a conductor and an insulator be \emph{explained} from scratch
--- derived from the charges present and their Coulomb interaction, with nothing further supplied?

Two formulations offer themselves. Textbook quantum mechanics represents the system by one
wavefunction on $3N$ dimensions, at a cost exponential in the number of electrons $N$; it cannot deliver
an answer at the size where the question exists, since the distinction is a property of an extended
system. RealQM represents it by $N$ non-negative unit charge densities in ordinary three-dimensional
space, one per electron, meeting at free boundaries and minimising the Coulomb energy, at polynomial
cost. It is computable, and it delivers the following.

\textbf{Polarisation, measured; sliding, not found.} On a regular chain of atoms with one valence
electron each, the polarisability $\alpha$ is extensive in the number of electrons: $\alpha=27.4N$, flat to $1\%$ per atom --- the response of charge fixed per
site, not of charge free to move between sites. And no sliding of the partition could be identified:
the corrugation it would have to pass through vanishes at no spacing, filling or boundary treatment
tested, placing the threshold field --- for the mode measured, a rigid slide on a clamped lattice
--- beyond dielectric breakdown. Both point to insulation.
...
Can the difference between a conductor and an insulator be \emph{explained} from scratch --- derived
from the charges present and their Coulomb interaction, with nothing further supplied?

Two formulations are available to be asked it, and they should be put side by side before either
is judged. The first is the one in every textbook: a single wavefunction on $3N$ dimensions
satisfying Schr\"odinger's equation, antisymmetric under exchange, from which everything else
follows. The second is RealQM.
...
Textbook quantum mechanics gives no answer to it. Not one that is merely out of reach for now ---
none at all, and not in principle either. Kohn established that the conductor/insulator distinction
follows from the ground state alone, with no transport theory required; what that does is convert
the question into a second one, produce the ground state. Solving the many-electron Schr\"odinger equation for that ground state costs
\textbf{exponentially} in the number of electrons, so its reach is a molecule and never a crystal,
and no growth in computing changes an exponent. Nor does the question live where the equation can be
solved: whether a response is bounded or unbounded is a statement about how it grows as the system
