...
\textbf{The corrugation is the activation energy.} It fixes the exponential in
$\sigma\propto\nu\exp(-E_a/kT)$, so $240$ against $24$~meV is four orders of magnitude in
conductivity at room temperature. The prefactor $\nu$ is a lattice frequency, which this framework
can also supply, since it moves nuclei under classical equations of motion --- though it is not
computed here. The metallic rate, needing a relaxation time from electron--phonon scattering, stays
outside.
\end{abstract}
