...
arithmetic. Measured time grows somewhat faster than the operation count: at a fixed $148^3$ grid on
one GPU, going from $32$ domains to $72$ multiplied the time per step by $3.3$ where the domain
count multiplied by $2.25$ --- an exponent near $1.5$, which is the price of carrying $N$ full-grid
potentials in memory rather than of the arithmetic. The precise exponent is not what matters here.
The single wave function of textbook quantum mechanics, by contrast, lives on a $3N$-dimensional
...
evidently closer to what the functional would choose. The scaling is the linear one of \S1 --- $N$ Poisson solves on one
grid --- so atoms are paid for once each and not compounded. At the mesh used here, $h=0.281\,a_0$, a $200^3$ grid spans $56\,a_0$ and holds
...
\paragraph{What the core domains omit.} The electron--electron term of \eqref{eq:E} excludes $i=j$,
and the solver enforces this by giving each domain the potential of every \emph{other} domain,
recomputed at every step. For a domain holding one electron that is exact: an electron does not repel
itself. The cable's core domains hold two, and two electrons sharing a domain do repel each other, so
the correct treatment retains a fraction $(Z-1)/Z=\half$ of the intra-domain repulsion. The path
used here removes the whole domain instead, so the cable's total energy is short by half the
intra-core repulsion at each of its sixteen sites --- several hartree out of $158$. It is stated
here because it is a definite omission rather than a discretisation error.

It does not reach any number quoted in this article, and the reason is the one that governs
\S3: the cores are held fixed while the carriers are displaced, so their shape, and
with it their intra-core repulsion, is identical at $d=0$ and at $d=\half$ and cancels exactly in
the difference. The barriers of \S5 and \S6 are therefore unaffected. What it does mean is that the
cable's total is not a physical total, which is consistent with the policy stated below: no binding
energy is quoted for the cable. The two binding energies that are quoted --- the chain's $-0.453$
and the lattice's $-0.524$~Ha per atom --- are for arrangements of one electron per domain, where
