...
\textbf{The last step is a compensation, not a parameter.} RealQM's free interface admits a flat
density at no cost, so its electron pressure vanishes --- $C=0$ exactly, the defect of
\S\ref{sec:coefficient} --- and the carriers are correspondingly too soft to resist being modulated
by the ions. Raising the coefficient of the kinetic energy from the $\tfrac12$ that atomic
calibration fixes to $1$ supplies the missing stiffness, and the corrugation falls elevenfold, to the
$20$~meV where conduction sets in. One deficiency, two symptoms: a wrong equation of state, and an
insulator that should conduct.
