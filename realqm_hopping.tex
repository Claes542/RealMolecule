...
\textbf{The answer depends on what the carrier is asked to be, and three arrangements give three
regimes.} On a chain of atoms with the valence electron sitting on its own kernel, the polarisability
is extensive, $\alpha=27.4N$ flat to $1\%$ per atom, and the corrugation the partition must slide
through is of order an electron-volt: an insulator, and outside the activated-hopping class this
framework claims. On a wire --- ion cores with the carriers standing off them, which is how matter is
actually built --- the corrugation falls to ${\approx}240$~meV, inside the $10$--$500$~meV band those
materials show. And when the coefficient of the kinetic energy is raised above the value $\tfrac12$ that atomic
spectra fix --- a value an extended object has no obligation to share --- it falls to
${\approx}24$~meV, thermal energy: the pinning no longer holds and the charge is free to move. Not as
a metal, since non-overlap leaves no extended states to fill into bands, but as a charge lattice
sliding collectively over the ions.
...
\section{Three geometries, three regimes}\label{sec:three}
...
\paragraph{The three regimes.} With the ion lattice, the standoff and the mesh held fixed:
\begin{center}
\small
\begin{tabular}{@{}lll@{}}
\toprule
...
\midrule
carrier on its own kernel      & ${\sim}1$~eV        & insulator; outside the hopping class\\
carrier standing off ion cores & ${\approx}240$~meV  & inside $10$--$500$~meV: activated hopping\\
carriers stiffened against modulation & ${\approx}24$~meV & thermal energy: unpinned\\
\bottomrule
\end{tabular}
\end{center}

\noindent The first two are geometry: the same framework, the same interaction, asked about two
different physical situations. The third is not a material but a diagnostic, and it is the
informative one. Doubling the carriers' resistance to being modulated --- with the ions, the standoff
