...
For nuclear motion the existing implementation already realises this scheme deterministically: classical Newton dynamics with a Langevin thermostat (the \texttt{USER\_DAMPING} and \texttt{langevinKT} parameters of \texttt{molecule.js}) samples the Boltzmann distribution by ergodic time-averaging, never computing $Z$ explicitly. The deterministic heat-equation extension of Section~\ref{sec:dissipation} adds an intrinsic finite-$T$ mechanism by coupling the wave dynamics to a parabolic heat field $T(x, \tau)$, with exact bookkeeping $E_{\rm wave} + E_{\rm thermal} = \mathrm{const}$.
