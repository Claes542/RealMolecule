...
\section{The Planck Cutoff as the FP 2nd Law}\label{sec:planck-fp2L}

The connection just stated can be made precise in the language of the
\textbf{FP 2nd Law} of \S\ref{sec:fp2L}. Recall the three load-bearing
clauses:

\begin{itemize}
\item \textbf{(i) Necessary / unavoidable.}  In a cavity at
temperature $T$, the high-frequency modes $\omega \gg k_{\mathrm{B}}T/\hbar$
are unstable to absorption by the walls --- the available thermal
energy per mode falls exponentially with $\omega$, and any quantum
deposited in such a mode is rapidly returned to the bath as
incoherent thermal motion of the wall material. The destruction of
the high-frequency tail is not optional: it is what \emph{must}
happen given the available resolution (the wall's microscopic
degrees of freedom) and the energy budget.

\item \textbf{(ii) Instability of fine-scale structure.}  The
``fine-scale structure'' here is the standing-wave field at very
short wavelength --- the radiative analogue of small-scale turbulent
eddies. As $\omega$ increases past $k_{\mathrm{B}}T/\hbar$, the wave
profile sustains gradients that the available resolution (the
photon-quantum scale $\hbar\omega$) cannot support coherently, and
the wave is absorbed.

\item \textbf{(iii) Irreversibility from precision deficit.}  Once
the energy that would have populated the high-frequency tail has
been deposited as heat in the walls, the precision required to
re-emit it back into a coherent high-frequency mode is not
available --- neither in the analog physical apparatus of the cavity
walls, nor in any digital simulator of the same field.
Re-population of the suppressed modes would require coordinating
the microscopic wall motion to produce coherent above-cutoff
radiation, which the precision of the wall material does not
support.
\end{itemize}

The Planck distribution is therefore not just an empirical curve
fitted to spectroscopic data, nor a quantum-statistical postulate; it
is the spectral signature of the FP 2nd Law applied to a continuum
electromagnetic field: the high-frequency suppression is the
necessary, unavoidable, irreversible destruction of unstable
fine-scale field structure that the available resolution cannot
support.
