...
\paragraph{Solid Ar --- the dispersion-only endpoint.}
The most extreme case --- monatomic, closed-shell, with no permanent multipole even in real life --- is solid Ar. A 2$\times$2$\times$2 FCC supercell (lattice 5.31~\AA, each Ar at +2 kernel rc=0.5 with 2 valence electrons in a hemi-split partition, no frozen outer shell) was simulated with the same protocol. Real Ar is essentially 100\% dispersion-bound (it does not solidify at standard pressure above $T_m = 84$~K). The Lindemann ratio crosses the threshold at $T_{\rm sub,sim} \approx 3000$~K, giving an overshoot factor $\sim$36$\times$. The residual cohesion in this fully homonuclear, fully spherical case is attributable to a \textbf{non-overlap stiffness} mechanism: even with closed-shell neutral atoms whose direct Coulomb interaction vanishes by Newton's shell theorem, compressing the inter-atomic Bernoulli boundaries costs kinetic energy from the resulting smaller territories. Architectural variations support this interpretation: a single multi-occupancy 2-electron orbital per atom (rigid orbital filling the whole atom region) gives stronger cohesion than a hemi-split partition (smaller, softer half-space orbitals), which falls apart earlier as expected for a softer system.
...
Ar solid             & $\sim$3000~K & 84~K   & $\sim$36$\times$  & $\sim$100\% dispersion \\
\bottomrule
\end{tabular}
...
The simulation transition T sits in the $\sim$1000--3000~K range across all four systems, reflecting the non-dispersive cohesion that the framework captures (full ionic Coulomb in NaCl, asymmetric-Bernoulli quadrupole in CO$_2$, multi-orbital residual quadrupole in N$_2$, non-overlap stiffness in Ar). The overshoot factor against experiment scales monotonically with the missing-dispersion fraction, providing a quantitative diagnostic for the regime of validity. The framework therefore both reproduces phase-transition character correctly and identifies, from the overshoot ratio alone, the dispersion contribution in any condensed-phase system. The Ar endpoint replaces the naive "100\%-missing-dispersion gives infinite overshoot" expectation with a finite floor set by the residual non-overlap stiffness intrinsic to the multiphase variational structure. Bulk water (216-water and 343-water boxes), studied at finite temperature with the same framework, reaches interactive frame rates on a single GPU; detailed bulk-water phase analysis is left to future work because dispersion (absent in RealQM at Level 3) becomes increasingly important for water structure at higher density.

\paragraph{Significance (overall).}
The combined NaCl/CO$_2$/N$_2$/Ar tests establish that the Coulomb-only formulation reaches solid-state physics, that its overshoot factor against experimental phase-transition temperatures is a coherent function of the missing-dispersion contribution, and that the computational cost is interactive rather than cluster-class. Condensed-phase physics is therefore within reach of the same single architecture that handles atoms, molecules, chemical reactions, and protein folding, with a quantitative way to diagnose the regime of validity.
...
Materials              & NaCl crystal melt              & T$_\mathrm{sim} \approx 1500$--$2000$~K & $T_m = 1074$~K & 1.4--1.9$\times$ overshoot                & \ref{sec:nacl}     \\
                       ...
                       & Ar solid (non-overlap stiffness) & T$_\mathrm{sim} \approx 3000$~K  & $T_m = 84$~K   & 36$\times$ ($\sim$100\% dispersion missing)    & \ref{sec:nacl}     \\
\bottomrule
\end{tabular}
