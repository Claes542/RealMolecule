...
costs to advance the electron pattern of a solid by one lattice site. No such computation without
fitted parameters appears to have been carried out, because the textbook $N$-electron state lives on
a $3N$-dimensional configuration space, so every practical route reduces the dimension and repairs
the reduction with modelled quantities --- a relaxation time, a transfer integral, an effective
mass.

The quantity is small against the total energy and is reached by differencing two configurations of
one system, so the large errors cancel. Four cases are computed. A \textbf{chain of sixteen
...
room-temperature $kT$. A \textbf{cubic lattice of seventy-two hydrogen atoms} yields no
barrier: scanned, its landscape descends away from registry, not a minimum there either.
