...
\paragraph{Ultraviolet cutoff from the viscous term.}
The Rayleigh--Jeans law alone gives an ultraviolet catastrophe ($\int \nu^2 d\nu \to \infty$). In Planck's resolution this is removed by quantising radiation as photons of energy $h\nu$. In the deterministic picture the cutoff is supplied instead by the viscous damping term $-\delta^2 u_{xxt}$ in~\eqref{eq:bbwave}: high-spatial-frequency modes are damped at a rate $\propto \delta^2 \nu^2$, which becomes comparable to the wave period and converts coherent oscillation into incoherent heat above a cutoff frequency $\nu_c \approx kT/h$ where $h$ is set by the resolution scale of the medium (not Planck's constant). The Planck spectrum
\begin{equation}
R_{\rm Planck}(\nu, T) \propto \frac{\nu^2 T}{1 + (\nu/\nu_c)^p}, \qquad \nu_c \approx kT/h
\end{equation}
then emerges as the Rayleigh--Jeans intensity multiplied by a cutoff factor whose physical origin is the second damping term, not quantisation. The blackbody acts as a high-pass filter: it re-emits frequencies below cut-off and stores frequencies above cut-off as heat through the $\delta^2$ channel.
