...
by $N$ real-valued wave functions on non-overlapping supports in three-dimensional space, meeting at
free interfaces. The physics is carried by one energy functional --- electron Coulomb potential
energy without self-repulsion, plus kinetic energy --- with nothing fitted. The cost of minimising it
scales as $N$ times the number of mesh points.
...
the prefactor with a rate theory puts the first two arrangements at $0.1$ and $11$~S\,m$^{-1}$,
bracketing intrinsic germanium, with attempt frequencies that come out at lattice values unasked.
The conductivities are lower bounds and the frequencies overestimates; the barriers are the computed
quantities.
\end{abstract}
