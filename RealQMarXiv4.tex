...
\normalsize\textit{Version 4: Hardy bound on kinetic energy and virial-residual diagnostic}}
...
The Bernoulli condition at the inter-electron boundary $\Gamma_i$ places the multiphase variational problem in a well-studied class of free-boundary problems for which existence and regularity have been established in closely related settings. The classical \emph{one-phase} Bernoulli problem (a single phase $\{u > 0\}$ with $u = 0$ and $|\nabla u|$ prescribed on $\partial\{u > 0\}$) was treated variationally by Alt and Caffarelli~\cite{AltCaffarelli81}; the two-phase extension by Alt, Caffarelli, and Friedman~\cite{AltCaffarelliFriedman84} introduced the structural template that generalizes to the $N$-phase RealQM setup. The most directly analogous results are by Caffarelli and Lin~\cite{CaffarelliLin08}, who treat $N$ non-negative phases with the singular-limit non-overlap constraint $u_i u_j = 0$ for $i \ne j$ and prove Lipschitz regularity of each $u_i$ at the partition boundary, $(n{-}1)$-rectifiability of the partition boundary, and $C^{1,\alpha}$ regularity of the boundary off a singular set where three or more phases meet. These results transfer to RealQM up to two adjustments: the wave functions $\psi_i$ are not constrained to be non-negative (but $\psi_i^2$ is, which is the relevant object), and the elliptic equation in each $\Omega_i$ is coupled to the others through the Hartree potentials $V_j$ rather than autonomous in $\psi_i$ alone. The eigenvalue / variational version closest in spirit to RealQM is the spectral minimal partition theory of Helffer, Hoffmann-Ostenhof, and Terracini~\cite{HHOT09}, where $\Omega$ is partitioned into $k$ subsets with the first Dirichlet eigenfunction on each; the partition-boundary structure (continuity of the eigenfunction across $\Gamma$ plus equal-magnitude gradient on each side) is exactly the Bernoulli condition of RealQM. Adapting these results to RealQM's coupled-Coulomb setting is the natural next step in the analytical theory.
...
\paragraph{Computation as constructive existence and regularity.}
The Hardy bound~\eqref{eq:bulk-bound} establishes stability of matter rigorously, but the deeper analytical questions about the multiphase variational problem of Section~\ref{sec:reformulation} --- existence and uniqueness of minimisers, regularity of the wave functions $\psi_i$ and of the free boundary $\Gamma_i$ at equilibrium --- are not currently within reach of analytical techniques. Standard tools for free-boundary problems in 3D (Caffarelli-type regularity theory, viscosity solutions, level-set regularity in the calculus of variations) apply in idealised geometries but do not obviously extend to the full $N$-electron Coulomb-with-non-overlap problem with mutually self-consistent Hartree potentials.

The numerical solutions reported in this paper carry part of the load that an analytical theory would otherwise carry. The 3-line code of Section~\ref{sec:3line}, applied to atoms, molecules, condensed phases, and protein-scale systems, reaches converged solutions in which the wave functions $\psi_i$ stabilise to spatial profiles consistent with the variational equations~\eqref{eq:Hi}--\eqref{eq:neumann}, the Bernoulli free boundary stabilises with continuity and homogeneous Neumann holding to mesh resolution, and the total energy plateaus. \textbf{Each such converged simulation may be regarded as a constructive proof, to a specified numerical precision, that a solution of the variational problem exists, and that it is regular enough to be represented on the simulation grid.} This is in the spirit of computer-assisted proofs (the 4-colour theorem, the Kepler conjecture, parts of mathematical fluid dynamics): a problem too complex for closed-form analytical resolution is settled, in the cases of interest, by an explicit construction whose correctness is verifiable. The constructive evidence does not replace eventual analytical work, but it carries the same epistemic load wherever such analytical work is not yet available --- and anyone who doubts that a particular system has a well-defined RealQM ground state can run the corresponding scan file from the public Gallery and inspect the converged result directly.
